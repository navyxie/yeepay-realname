...
var p0_Cmd_Query = "FastRealNameAuthSearch";//查询指令,常量。
...
*return string
*/
YeepayRealName.prototype._getAuthorizationURL = function(){
...
YeepayRealName.prototype.authorize = function(data,cbf){
	data = data || {};
	cbf = cbf || util.noop;
	data = util.extend(data,this._getConstans(data));
	...
	// }	
	var url = this._getAuthorizationURL();
	var formData = {
...
*查询实名认证订单接口
*/
YeepayRealName.prototype.query = function(data,cbf){
	data = util.extend(data,this._getConstans(data));
	if(!data.busId){
		return cbf({code:-2,msg:"参数不正确，缺少商户订单号(busId)！"});
	}	
	var hmac = this._getQueryMd5Hmac(data);
	var formData = {
		p0_Cmd:data.p0_Cmd_Query,
		customerId:toString(data.customerId),
		busId:toString(data.busId),
		flowId:toString(data.flowId || ""),
		hmac:hmac
	};
	var url =  digest.encodeGBK(this._getAuthorizationURL()+'?'+util.jsonToSearch(formData));//这一步很重要，对中文utf8进行GBK转码
	request.get(url,function(err,httpResponse,body){
		if(err){
			cbf({code:-3,msg:'请求易宝实名认证接口失败了。',err:err,url:url});
		}else{
...
/**
*根据提交的数据，获取查询实名认证订单MD5签名
*@param object jsonData
*return string 
*/
YeepayRealName.prototype._getQueryMd5Hmac = function(jsonData){
	if(!util.checkQueryParam(jsonData)){
		return "";
	}
	var str = "";
	str += jsonData.p0_Cmd_Query;
	str += jsonData.customerId;
	str += jsonData.busId;
	jsonData.flowId ? (str += jsonData.flowId) : (str += "");
	return digest.hmacSign(str,this.config.keyValue);
}
